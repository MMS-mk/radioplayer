...
if(!empty($url)) {
	if($streamingType === 'shoutcast') {
		...
		if(!empty($data)) {
			...
			// максимум број на слушатели
			$array['listenersPeak'] = $streamingData[3];
			// колку имало најмногу слушатели
			$array['listeners'] = $streamingData[4];
			// на колкава фреквенција се емитува стимот
			$array['transmissionFrequency'] = $streamingData[5];	
			// што свири во моментот
			...
			// која песна свири во моментот
			$currentSong = explode('-', $playingNow);
			// кој е изведувач на песната
			$artist = trim($currentSong[0]);
			// менење на HTML кодот со изведувач и наслов на песната
		...
		// за ICECAT 2.4 само (постарите верзии имаат проблем со XSL фаилот)
		...
		$array = ['error' => 'STREAM_TYPE параметарот не е најден'];
	}
} else {
	$array = ['error' => 'URL не ти е точно'];
}
